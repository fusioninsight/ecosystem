#!/bin/bash
# 功能：在本节点上停止telnet服务，关闭端口
# 关闭端口后，负载均衡器探测不到该端口，就会认为此节点是不可用的
# 返回值：0 启动成功, 1 启动失败
#
#
...
function main()
{
    ...
    if [ $? -ne 0 ]; then
        LOG "INFO" "telnet server has stop."
        return 0
    else 
       sudo systemctl stop ${process_name}
        if [ $? -eq 0 ]; then
            LOG "INFO" "telnet server stop success."
            return 0
        else
            LOG "ERROR" "telnet server stop failed."
            return 1
        fi
    fi
}
