...
* 配置OpenTSDB
  ...
  * 修改opentsdb的tsdb脚本增加认证配置。
    ...
    将`/usr/local/share/opentsdb/bin/tsdb`脚本倒数第二行（else后面）的“exec $JAVA $JVMARGS -classpath "$CLASSPATH" net.opentsdb.tools.$MAINCLASS "$@" ”修改为 **exec $JAVA $JVMARGS -classpath "$CLASSPATH" -Djava.security.krb5.conf=/etc/opentsdb/krb5.conf -Djava.security.auth.login.config=/etc/opentsdb/jaas.conf -Dzookeeper.server.principal=zookeeper/hadoop.hadoop.com net.opentsdb.tools.$MAINCLASS "$@"** 。
