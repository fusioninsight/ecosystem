...
* 软件获取
  ...
  * 替换hdp26\lib目录下Hive相关的jar包以及hdp26\lib\client目录下hdfs相关的jar包
  ...
  ![](assets/Kettle_8.x/f2c7a.png)

  ![](assets/Kettle_8.x/62015.png)

  并添加以下两个包
  ![](assets/Kettle_8.x/674e5.png)
...
* 为连接命名，连接类型选择Hive 2，填写主机名、端口号、数据库名
  ...
  ![](assets/Kettle_8.x/image05.png)

* 点击左侧 **选项**，填写以下参数：

  ![](assets/Kettle_8.x/image06.png)

* 点击测试，显示以下窗口，表明测试成功

  ![](assets/Kettle_8.x/image07.png)
...
* 添加转换步骤
  ...
  ![](assets/Kettle_8.x/image08.png)

* 修改Hive表输入配置

  双击 **表输入** 步骤， **数据库连接** 选择前面创建的hive连接，点击 **获取SQL查询语句** ，选择需要导入的hive表

  ![](assets/Kettle_8.x/image09.png)

  选择`是`,该表的字段将会包含在SQL语句中，

  ![](assets/Kettle_8.x/image10.png)
...
* 修改文本文件输出配置
  ...
  ![](assets/Kettle_8.x/image11.png)

  在内容选项卡中，设置文件输出时属性
  ![](assets/Kettle_8.x/image12.png)

  在字段选项卡中，点击`获取字段`，获得文件字段内容，可以点击`最小宽度`,使字段宽度最小
  ![](assets/Kettle_8.x/image13.png)

  点击`确定`,保存设置。

* 运行转换，在主界面点击工具栏左侧的三角形运行按钮

  ![](assets/Kettle_8.x/image14.png)

  执行结果：

  ![](assets/Kettle_8.x/image15.png)
...
* 运行转换
  ...
  ![](assets/Kettle_8.x/image21.png)

  查看Hive表数据：

  ![](assets/Kettle_8.x/image22.png)
...
Clusters** 右键选择 **New Cluster**
  ...
  * Oozie的URL填写oozie WebUI的地址.
  ...
  ![](assets/Kettle_8.x/image23.png)

  点击 **测试**

  ![](assets/Kettle_8.x/image24.png)
...
* 修改Hadoop File Output配置
  ...
  ![](assets/Kettle_8.x/image26.png)

* 点击 **内容** 页签，设置分隔符，勾选 **快速数据存储（无格式）**（否则保存的文件中会按字段长度填充空格）

  ![](assets/Kettle_8.x/image27.png)

* 点击 **字段** 页签，获取字段，并设置最小宽度

  ![](assets/Kettle_8.x/image28.png)
...
* 执行结果：
  ...
  ![](assets/Kettle_8.x/image29.png)

  查看HDFS文件

  ![](assets/Kettle_8.x/image30.png)
...
* 添加转换步骤
  ...
  ![](assets/Kettle_8.x/image31.png)

* 修改 Hadoop File Input配置

  双击 **Hadoop File Input** 步骤，**文件** 页签，选择待导出的文件，文件类型支持CSV（txt也可以）和Fixed（固定列宽）

  ![](assets/Kettle_8.x/image32.png)

  点击 **内容** 页签，选择文件类型、分隔符、编码方式等

  ![](assets/Kettle_8.x/image33.png)

  点击 **字段** 页签，获取字段

  ![](assets/Kettle_8.x/image34.png)
...
* 修改Microsoft Excel输出配置
  ...
  ![](assets/Kettle_8.x/image35.png)

  点击 **字段** 页签，获取字段

  ![](assets/Kettle_8.x/image36.png)
...
* 执行结果
  ...
  ![](assets/Kettle_8.x/image37.png)

* 查看导出的excel文件

  ![](assets/Kettle_8.x/image38.png)
...
* 修改 **HBase Output** 配置
  ...
  ![](assets/Kettle_8.x/image40.png)

* 若该表没有创建mapping,在 **Create/Edit Mappings** 页签创建mapping,指定各项属性

  ![](assets/Kettle_8.x/image41.png)
...
* 修改文本文件输出配置
  ...
  ![](assets/Kettle_8.x/image48.png)

  点击 **字段** 页签，点击`获取字段`,设置最小宽度(可选)

  ![](assets/Kettle_8.x/image49.png)
...
* 执行结果
  ...
  ![](assets/Kettle_8.x/image50.png)

* 查看导出的文件

  ![](assets/Kettle_8.x/image51.png)
